...
// Feather distribution queue: player names ranked by Total Points (Points tab).
// Column names are detected leniently so it survives header variations.
export async function getFeatherQueue() {
  const rows = await getSheet('Points')
  if (!rows.length) return []
  const headers = Object.keys(rows[0])
  const nameKey =
    headers.find(h => /display\s*name/i.test(h)) ||
    headers.find(h => /\b(name|player|ign|member|user)\b/i.test(h)) ||
    headers[0]
  const pointsKey =
    headers.find(h => /total\s*points/i.test(h)) ||
    headers.find(h => /points/i.test(h))

  const queue = rows
    .map(r => ({
      name: String(r[nameKey] || '').trim(),
      points: parseFloat(String(r[pointsKey] || '0').replace(/,/g, '')) || 0,
    }))
    .filter(p => p.name)

  if (pointsKey) queue.sort((a, b) => b.points - a.points)
  return queue.map(p => p.name)
}
