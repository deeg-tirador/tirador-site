...
// Reuse a single authorized client across requests on a warm serverless
// instance (avoids re-creating the JWT and re-authing on every read).
...
// In-memory cache keyed by tab name. Persists across requests handled by the
// same warm instance, so a burst of page loads shares one API read instead of
// hammering the Sheets quota (the cause of event-night 500s).
const CACHE_TTL_MS = 45 * 1000
const _cache = new Map() // tabName -> { ts, data }
const _inflight = new Map() // tabName -> Promise (dedupe concurrent reads)
...
async function getSheet(tabName) {
  const now = Date.now()
  const cached = _cache.get(tabName)
  if (cached && (now - cached.ts) < CACHE_TTL_MS) return cached.data

  // De-dupe concurrent reads of the same tab into one API call.
  if (_inflight.has(tabName)) return _inflight.get(tabName)

  const p = (async () => {
    try {
      const data = await fetchTab(tabName)
      _cache.set(tabName, { ts: Date.now(), data })
      return data
    } catch (err) {
      // On failure, fall back to any stale cache rather than 500-ing the page.
      const stale = _cache.get(tabName)
